      ...
      <div className="flex-1 flex flex-col h-full bg-zinc-50 dark:bg-zinc-950 overflow-hidden min-h-0">
        ...
        <div className="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
          ...
          <div className="flex justify-between items-center px-1 shrink-0">
             ...
             <div className="flex bg-zinc-200 dark:bg-zinc-800 p-1 rounded-lg">
                {['Daily', 'Weekly', 'Monthly', 'Yearly'].map((range) => (
                  <button key={range} onClick={() => setInsightTimeRange(range as TimeRange)} className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wide rounded-md transition-all ${insightTimeRange === range ? `bg-white dark:bg-zinc-600 shadow-sm ${themeStyles.text} dark:text-white` : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200'}`}>{range}</button>
                ))}
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 shrink-0">
             {/* Deep Work */}
             <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                 <div>
                    <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Deep Focus</span>
                    <div className="text-xl font-bold dark:text-white mt-1">{analytics.deepWorkHours}h</div>
                 </div>
                 <div className={`p-2 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-500`}><Zap size={18} /></div>
             </div>
             
             {/* Total Time */}
             <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                 <div>
                    <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Total Time</span>
                    <div className="text-xl font-bold dark:text-white mt-1">{analytics.totalHours}h</div>
                 </div>
                 <div className={`p-2 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-500`}><Clock size={18} /></div>
             </div>
             
             {/* Completion */}
             <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                 <div>
                    <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Completion</span>
                    <div className="text-xl font-bold dark:text-white mt-1 flex items-baseline gap-1">
                      {analytics.completionRate}%
                      <span className="text-xs text-zinc-400 font-normal">({analytics.completed}/{analytics.total})</span>
                    </div>
                 </div>
                 <div className={`p-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500`}><CheckCircle2 size={18} /></div>
             </div>
             
             {/* Mood */}
             <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                 <div>
                    <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Avg Mood</span>
                    <div className="text-xl font-bold dark:text-white mt-1">{analytics.avgMood}</div>
                 </div>
                 <div className={`p-2 rounded-xl bg-purple-50 dark:bg-purple-900/20 text-purple-500`}>
                    {Number(analytics.avgMood) >= 4 ? <Smile size={18} /> : Number(analytics.avgMood) >= 3 ? <Meh size={18} /> : <Frown size={18} />}
                 </div>
             </div>
          </div>
             ...
             <div className="flex-[3] bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800 flex flex-col min-h-[220px]">
                ...
                <div className="flex-1 w-full min-h-0">
                   <ResponsiveContainer width="100%" height="100%">
                     <BarChart data={analytics.barData} margin={{ top: 5, right: 5, left: -25, bottom: 25 }} barGap={0}>
                        ...
                        <Bar dataKey="pending" stackId="a" radius={[4, 4, 0, 0]} barSize={40}>
                           {analytics.barData.map((entry, index) => <Cell key={`p-${index}`} fill={entry.color} fillOpacity={0.3} />)}
                        </Bar>
                     </BarChart>
                   </ResponsiveContainer>
                </div>
                   ...
                   {analytics.topActivities.length > 0 ? analytics.topActivities.map((item, idx) => (
                     <div key={idx} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                        <div className="flex items-center gap-3">
                           ...
                           <div className="opacity-70" style={{ color: item.color }}>
                              {getCategoryIcon(item.icon, 16)}
                           </div>
                           <span className="text-sm font-semibold dark:text-zinc-200">{item.name}</span>
                        </div>
                        <span className="text-xs font-mono font-bold text-zinc-500 dark:text-zinc-400">{item.hours}h</span>
                     </div>
                   )) : (
                     <div className="h-32 flex flex-col items-center justify-center text-zinc-400 text-xs italic">
          ...
          <div className="flex-[1.5] min-h-[250px] flex flex-col bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 overflow-hidden shrink-0">
             ...
             <div className="grid grid-cols-7 gap-px bg-zinc-200 dark:bg-zinc-800 flex-1">
               {daysInWeek.map((day, i) => {
                  ...
                  // Calculate focus hours for this day
                  let focusMinutes = 0;
                  dayTasks.forEach(t => {
                     const cat = categories.find(c => c.id === t.categoryId);
                     if(cat?.isFocus && t.completed) {
                       const s = new Date(`${t.date}T${t.startTime}`);
                       let e = new Date(`${t.date}T${t.endTime}`);
                       if(e < s) e = addDays(e, 1);
                       focusMinutes += differenceInMinutes(e, s);
                     }
                  });
                  ...
                  return (
                    <button 
                      ...
                      className={`relative flex flex-col text-left transition-all h-full overflow-hidden bg-white dark:bg-zinc-950 hover:bg-zinc-50 dark:hover:bg-zinc-900 group`}
                    >
                      <div className={`p-2 flex flex-col border-b border-zinc-100 dark:border-zinc-900 ${isSelected ? 'bg-zinc-50 dark:bg-zinc-900' : ''}`}>
                         <div className="flex justify-between items-start w-full">
                            <div>
                                <span className={`block text-xs font-black uppercase tracking-wider ${isSelected ? themeStyles.text : 'text-zinc-400'}`}>{format(day, 'EEE')}</span>
                                <div className={`text-3xl font-bold mt-0.5 leading-none ${isSelected ? themeStyles.text : isToday ? themeStyles.text : 'dark:text-zinc-300'}`}>
                                   {format(day, 'd')}
                                   {isSelected && <span className={`ml-1.5 inline-block w-1.5 h-1.5 rounded-full ${themeStyles.bg} align-middle mb-1`}></span>}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-end gap-1">
                                {dayLog ? (
                                    <div className={`${getScoreColor(dayLog.score)} flex items-center`} title={`${dayLog.score}/5 Mood`}>
                                      {dayLog.score >= 4 ? <Smile size={16}/> : dayLog.score === 3 ? <Meh size={16}/> : <Frown size={16}/>}
                                      <span className="ml-1 text-[10px] font-bold text-zinc-500">{dayLog.score}</span>
                                    </div>
                                ) : (
                                    <div className="text-zinc-300 dark:text-zinc-700" title="No Mood Score"><Circle size={16} className="stroke-2 opacity-50 stroke-dashed"/></div>
                                )}
                                
                                <div className="flex flex-col items-end gap-0.5 text-[10px] text-zinc-400 font-medium">
                                    {dayTasks.length > 0 && (
                                      <span className={`flex items-center gap-0.5 ${completionRate === 100 ? 'text-emerald-500' : ''}`}>
                                        <CheckCircle2 size={10}/> {completionRate}%
                                      </span>
                                    )}
                                    {focusHours > 0 && (
                                      <span className="flex items-center gap-0.5 text-amber-500">
                                        <Zap size={10}/> {focusHours.toFixed(1)}h
                                      </span>
                                    )}
                                </div>
                            </div>
                      ...
                      <div className="flex-1 p-1 overflow-y-auto no-scrollbar space-y-1">
                         {dayTasks.slice(0, 4).map(t => (
                           <div key={t.id} className="flex items-start gap-1.5 py-1">
                             <div className={`text-zinc-400 shrink-0 mt-0.5 ${t.completed ? 'opacity-40' : ''}`}>
                               {getCategoryIcon(categories.find(c=>c.id===t.categoryId)?.icon || categories.find(c=>c.id===t.categoryId)?.id, 10, t.completed ? "" : categories.find(c=>c.id===t.categoryId)?.color ? `text-[${categories.find(c=>c.id===t.categoryId)?.color}]` : "")}
                             </div>
                             <span className={`text-[11px] leading-tight line-clamp-2 ${t.completed ? 'text-zinc-300 line-through' : 'text-zinc-500 dark:text-zinc-400'}`}>{t.title}</span>
                           </div>
                         ))}
                         {dayTasks.length > 4 && <div className="text-[10px] text-zinc-300 pl-2">+{dayTasks.length - 4} more</div>}
                      </div>
                    </button>
                  )
               })}
